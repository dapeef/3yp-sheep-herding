...
def navigate_loop(points, drone1_pos,drone2_pos,drone3_pos,drone4_pos):
    ...
    drone3_pos_arr = np.array(drone3_pos)
    drone4_pos_arr = np.array(drone4_pos)
    ...
    drone3_index = np.argmin(np.sum((points_arr - drone3_pos_arr)**2, axis=1))
    drone4_index = np.argmin(np.sum((points_arr - drone4_pos_arr)**2, axis=1))

    #check desired relative positions and assign targets
    length = len(points)
    if drone1_index >= (drone4_index + length//4) % length:
        next_index1 = (drone1_index + 1) % length
    else:
        next_index1 = (drone1_index + 2) % length
    if drone2_index >= (drone1_index + length//4) % length:
        next_index2 = (drone2_index + 1) % length
    else:
        next_index2 = (drone2_index + 2) % length
    if drone3_index >= (drone2_index + length//4) % length:
        next_index3 = (drone3_index + 1) % length
    else:
        next_index3 = (drone3_index + 2) % length
    if drone4_index >= (drone3_index + length//4) % length:
        next_index4 = (drone4_index + 1) % length
    else:
        next_index4 = (drone4_index + 2) % length

    target_indices = [next_index1,next_index2,next_index3,next_index4]

    # Check for overlapping drones and adjust target points accordingly
    for i in range(4):
        for j in range(i+1, 4):
            if np.allclose(eval(f"drone{i+1}_pos_arr"), eval(f"drone{j+1}_pos_arr"), atol=0.1):
                if target_indices[i] == target_indices[j]:
                    target_indices[j] = (target_indices[j] + 1) % length
                target_indices[i] = (target_indices[i] - 1) % length

    # Compute the coordinates of the next points
    next_pos1 = tuple(points[next_index1])
    next_pos2 = tuple(points[next_index2])
    next_pos3 = tuple(points[next_index3])
    next_pos4 = tuple(points[next_index4])
